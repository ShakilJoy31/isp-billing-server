...
const createBankAccount = async (req, res, next) => {
  try {
    ...
    if (!bankName || !accountHolderName || !accountName || !accountNumber || !accountType) {
      return res.status(400).json({ 
        success: false,
        message: "Missing required fields: bankName, accountHolderName, accountName, accountNumber, accountType" 
      });
    }
...
const getAllBankAccounts = async (req, res, next) => {
  try {
    ...
    const validSortFields = [
      'accountHolderName', 'bankName', 'accountType', 'currentBalance', 
      'openingBalance', 'createdAt', 'updatedAt', 'lastTransactionDate'
    ];
...
const updateAccountBalance = async (req, res, next) => {
  try {
    ...
    if (!transactionType || !['deposit', 'withdrawal', 'transfer'].includes(transactionType)) {
      return res.status(400).json({
        success: false,
        message: "Valid transaction type is required (deposit, withdrawal, transfer)"
      });
    }
    ...
    if (transactionType === 'deposit') {
      ...
      if (account.monthlyLimit && transactionAmount > account.monthlyLimit) {
        return res.status(400).json({
          success: false,
          message: "Transaction amount exceeds monthly limit"
        });
      }
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        previousBalance: account.currentBalance - (transactionType === 'deposit' ? -transactionAmount : transactionAmount),
...
const getBankAccountStats = async (req, res, next) => {
  try {
    ...
    const activeAccounts = await BankAccount.count({ where: { isActive: true } });
    const primaryAccounts = await BankAccount.count({ where: { isPrimary: true } });
    ...
    const totalBalance = totalBalanceResult ? totalBalanceResult.dataValues.totalBalance : 0;
...
const getAccountsByType = async (req, res, next) => {
  try {
    ...
    const validTypes = ['Bank', 'MobileBanking', 'AgentBanking', 'DigitalWallet', 'Other'];
    
    if (!validTypes.includes(type)) {
      return res.status(400).json({ 
        success: false,
        message: "Invalid account type. Valid types: Bank, MobileBanking, AgentBanking, DigitalWallet, Other" 
      });
    }
    ...
    const accounts = await BankAccount.findAll({
      ...
      order: [['bankName', 'ASC'], ['accountHolderName', 'ASC']]
    });
